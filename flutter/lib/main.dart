...
class ConceptPage extends StatelessWidget {
  ...
  @override Widget build(BuildContext context)=>ListView(padding:const EdgeInsets.all(24),children:[const SizedBox(height:40),Text('最大7名から、\nひとりを選ぶ。',style:Theme.of(context).textTheme.displaySmall?.copyWith(fontFamily:'serif',fontWeight:FontWeight.bold)),const SizedBox(height:20),const Text('男性ホストはZEUS、女性ホストはAPHRODITE。候補者6名でも成立する検証条件です。',style:TextStyle(color:Color(0xFFB9AFB4),height:1.7)),const SizedBox(height:24),const _Card(title:'ZEUS',body:'男性ホスト1名＋女性候補者6〜7名'),const _Card(title:'APHRODITE',body:'女性ホスト1名＋男性候補者6〜7名'),const _Card(title:'PRIVACY',body:'候補者同士はAI似顔絵。ホストと候補者間だけ実写。'),const SizedBox(height:12),const Card(child:Padding(padding:EdgeInsets.all(16),child:Text('検証版では課金・eKYC・実デートを行いません。'))) ]);
}
...
class _JoinPageState extends State<JoinPage>{String gender='male',role='host',age='25-29',message='';bool portrait=false,busy=false;Future<void> submit()async{setState(()=>busy=true);try{final id=await widget.api.register(gender:gender,target:gender=='male'?'female':'male',role:role,ageBand:age,portraitOptIn:portrait);setState(()=>message='匿名ID: $id');}catch(e){setState(()=>message='API接続エラー: $e');}finally{setState(()=>busy=false);}}@override Widget build(BuildContext c)=>ListView(padding:const EdgeInsets.all(20),children:[Text('匿名プロフィール',style:Theme.of(c).textTheme.headlineMedium),DropdownButtonFormField(value:gender,items:const [DropdownMenuItem(value:'male',child:Text('男性')),DropdownMenuItem(value:'female',child:Text('女性'))],onChanged:(v)=>setState(()=>gender=v!)),DropdownButtonFormField(value:role,items:const [DropdownMenuItem(value:'host',child:Text('ホスト')),DropdownMenuItem(value:'candidate',child:Text('候補者'))],onChanged:(v)=>setState(()=>role=v!)),DropdownButtonFormField(value:age,items:['20-24','25-29','30-34','35-39'].map((x)=>DropdownMenuItem(value:x,child:Text(x))).toList(),onChanged:(v)=>setState(()=>age=v!)),SwitchListTile(value:portrait,onChanged:(v)=>setState(()=>portrait=v),title:const Text('AI似顔絵テストを希望')),FilledButton(onPressed:busy?null:submit,child:Text(busy?'登録中…':'同意して匿名登録')),SelectableText(message)]);}
